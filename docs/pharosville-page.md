...
- ship scale uses exaggerated compressed market-cap tiers, not linear supply area, so $1B+ issuers are spottable while USDC, USDS, and USDT receive dedicated titan-size hull treatments instead of linear supply area; the five Maker stables (USDS as flagship, plus DAI, sUSDS, sDAI, stUSDS as consorts) sail in a fixed Sky/Maker squad that shares the flagship's risk placement and motion cycle, with stUSDS leading as the risk-capital icebreaker, savings cutters flanking, and DAI trailing port-aft as veteran consort; a world-space golden bunting connects mast-tops as the squad's cohesion signal, and any consort whose own peg/stress signal exceeds the flagship's surfaces a "<symbol> in distress — squad sheltering at flagship's position" banner in the detail panel and accessibility ledger
