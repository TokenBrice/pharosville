...
| Dock atlas placement | `systems/chain-docks.test.ts` | Ethereum/L2 preferred harbors and ten-dock chain harbor cap | `npm test -- src/systems/chain-docks.test.ts` |
...
| Motion route behavior | `systems/motion.test.ts` | Normal-motion routes, route cycles, all six risk-water zones, meaningful dockless patrols, ordered DEWS dwell/wake/drift, danger/Ledger proximity, and water-only samples | `npm test -- src/systems/motion.test.ts` |
...
| Hit target alignment | `renderer/hit-testing.test.ts` | Manifest hitboxes, moving ship targets, docked ships, and absence of retired building selection targets | `npm test -- src/renderer/hit-testing.test.ts` |
