...
fixture: 10 docks, all 132 current dense-fixture active ships rendered
...
| Dense visual atlas | Playwright `pharosville dense visual fixture preserves districts, dense ships, and render budget` | District density, coherent ship sprites, 10 docks, 132 dense-fixture ship motion samples, rotating normal-motion visible ship targets, no ship-cluster targets, cemetery/civic/risk-water crops, and normal-motion p95 draw budget | `npx playwright test tests/visual/pharosville.spec.ts --grep "dense visual fixture"` |
