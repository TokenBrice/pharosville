...
| Dense Ledger northeast crop | `pharosville dense visual fixture preserves districts, dense ships, and render budget` | `tests/visual/pharosville.spec.ts-snapshots/pharosville-dense-ledger-northeast-linux.png` | Ledger Mooring on the northeast shelf between Watch Breakwater and the elevated Alert/Warning/Danger stack, with NAV ships visibly moored and labels readable. |
...
- The DEWS sea reads as a clear escalation route matching the current compound masks: Calm Anchorage fills the large left-edge and reclaimed south/right basin, Watch Breakwater owns the remaining top-edge band, Alert Channel, Warning Shoals, and Danger Strait occupy overlapping eastern-corner rings with Danger snapped to the angled shelf, and Ledger Mooring reads as quiet ledger water on the northeast shelf beside but not over the alert stack.
