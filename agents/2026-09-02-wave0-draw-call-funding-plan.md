...
- **Slot allocation.** A fleet-batch slot is LOCAL to its silhouette (`writeFleetInstance` picks the silhouette, then `batch.hull.mesh.count`, `garden-fleet-batch.ts` ≈1398–1401) and collides across families and with hero indices — it must NOT be used. In `buildShipsPart`, allocate one unique `wakeSlot: number` per ship in stable `content.ships` order, store it on every `ShipVisual` (heroes included — they have wakes too), and use only that for the batch. **Capacity is not `GARDEN_FLEET_BATCH_CAPACITY`:** the live fleet is sized to that cap, so reserving a slot inside it would evict a real ship's wake. Size the batch exactly as the ship shadows already do — `createShipShadows(ships.length + departingShips.length + 1)` at `world-renderer.ts` ≈3273–3277 — i.e. `ships.length + departing.length + 1`, with departing ships and the transient selected outsider taking their own slots beyond the live fleet. Never overwrite or skip an eligible ship's wake.
