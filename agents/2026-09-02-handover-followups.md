...
- Moored display composition (root of the "moored hull on the paper" frame): a representative hull's moored tile was `mooringTile + displayOffset`, i.e. the blue-noise berth offset carried along a data-space voyage, up to ~100 tiles from the quay. `gardenHomeOffsetWeight` now fades the offset from 1 inside the home patrol radius (6 tiles) to 0 at the nearest mooring reach, so moored hulls sit at their station berth and the sail out is continuous.
