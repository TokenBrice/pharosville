...
| U1.5 | Remove auto-day-night: `autoNightCycle` state, its `setInterval`, `toggleAutoNightCycle`, and the control entry | `hooks/use-world-time-controls.ts:25,41-45,91-95`, `content/pharosville-controls.ts:160-165` | no interval remains; the canonical runtime guide retains the no-independent-timer rule |
...
| U8.6 | Update the canonical change checklist if the disclosure introduces a focus-order trap, and `AGENT_ONBOARDING.md` routing if component names changed | `npm run check:doc-paths-and-scripts` |
