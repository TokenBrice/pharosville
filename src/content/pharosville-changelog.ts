...
export const PHAROSVILLE_CHANGELOG: PharosVilleChangelogEntry[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Cut the cost of being left open. Ships hold their tiles across refreshes unless their risk placement actually changed, which stops refresh teleporting and takes the world rebuild from 303ms to 34ms; runtime models are meshopt-compressed by half with a maximum geometry shift of 2.44e-4 units; the frame duty halves after three minutes without input; and logo requests stop revalidating on every visit. A real-GPU measurement of the refresh \u2014 0ms, 239ms and 795ms of main-thread work for an identical, a typical and a fully churned payload \u2014 ruled out a Web Worker rather than estimating it.",
      "Made the gates tell the truth and widened what they cover. npm run preview --assert is now a deploy gate that fails rather than reports a software frame, the dist visual and perf lanes reach their assertions after long-stale expectations and a fixture ship that never existed, a refresh-soak gate cycles 12 payloads and requires renderer memory to return to baseline, the world-data hook and the enrichment grace path have their first tests, and the media guard now validates that referenced media can actually render \u2014 which found a logo truncated and rendering blank since the bootstrap commit.",
    ...
    source: "Collected from commits 871408a through 361a03c after the v0.4.0 changelog entry.",
  },
  {
  ...
  },
  {
    ...
    date: "2026-07-10",
    title: "True Waters",
    summary: "PharosVille learns to tell the whole truth: inverted stability semantics, dead permalinks, and silent story beats are fixed, the harbor finally leads the first visit, and ships sail with honest wakes and cadence.",
    bullets: [
      ...
      "Made the world the hero on first visit — one narrow legend (with hull-class thumbnails) over a visible harbor, the lighthouse panel deferred until it closes, Watch Breakwater water retuned to a distinct steel teal, a quieter footer with the fps counter behind ?debug=1.",
    ...
    source: "Collected from commits c02952a through 654ea7f after the v0.2.2 changelog entry.",
  },
  {
