...
// Generated by scripts/pharosville/extract-brand-colors.mjs from the issuer
// logo files; regenerate when logos change. Primaries are pre-clamped into a
// livery-safe lightness window and pre-spread by an OKLab distance guard.
// Entries are packed "#primary|#secondary" to keep the bundled JSON small.
const BRAND_COLORS = brandColors as Record<string, string>;
...
export function resolveStablecoinShipBranding(id: string, meta: StablecoinMeta): StablecoinShipBranding {
  return STABLECOIN_SAIL_COLORS[id]
    ?? brandDerivedLivery(id)
    ?? derivedFallbackLivery(id, meta);
}

// Identity pass P3 — brand-color-first livery. The issuer's extracted brand
// primary drives every colored surface (sail cloth tint, pennant, trim,
// stripes) so ships separate by hue at a glance instead of converging on the
// per-peg pastel. Shapes/panels/stripes stay hash-derived for silhouette
// variety. Falls through to the peg-derived livery when no brand color was
// extracted for the id.
function brandDerivedLivery(id: string): StablecoinShipBranding | null {
  const packed = BRAND_COLORS[id];
  if (!packed) return null;
  const [primary, secondary] = packed.split("|") as [string, string];
  if (!primary || !secondary) return null;
  const hash = stableFnv1aHash(id);
  const accent = secondary === primary ? lightenHex(primary, 0.34) : secondary;
  return {
    accent,
    label: "Issuer brand livery",
    logoMatte: lightenHex(primary, 0.85),
    logoShape: FALLBACK_LOGO_SHAPES[(hash >>> 5) % FALLBACK_LOGO_SHAPES.length],
    primary,
    sailColor: mixHex(primary, "#f5efdc", 0.6),
    sailPanel: FALLBACK_SAIL_PANELS[(hash >>> 9) % FALLBACK_SAIL_PANELS.length],
    secondary: darkenHex(primary, 0.46),
    source: "brand-color",
